...
def solid_setup(d_, v_, phi, psi, dx_s, mu_s, rho_s, lambda_s, k, theta,
    ...
    if isinstance(mu_s, list): # If there are multpile solid regions
        F_solid_linear = 0
        F_solid_nonlinear = 0
    ...
    else: # If only one solid region is defined
        # Temporal term and convection
        ...
        # Gravity
        if gravity is not None and mesh.geometry().dim() == 2: # If it's a 2D problem:
            F_solid_linear -= inner(Constant((0, -gravity * rho_s)), psi)*dx_s 
        elif gravity is not None and mesh.geometry().dim() == 3: # If it's a 3D problem:
            F_solid_linear -= inner(Constant((0, -gravity * rho_s,0)), psi)*dx_s 
